<template>
  ...
  >
    <div style="text-align: center;">
      <el-image
        :src="avatar"
        style="width: 150px;height: 150px;margin-bottom: 10px;border-radius: 9px;"
      >
      </el-image>
      <div
        style="left:50%;top:50%;width: 100px;height: 100px;z-index: 99;position:fixed;"
        v-loading="loading"
      ></div>
    </div>
    <el-card
      :body-style="{ padding: '10px' }"
      style="margin-bottom: 10px;height: 80%;"
    >
      <div
        style="white-space: pre;line-height: 50px;font-family: 'Microsoft YaHei',sans-serif"
      >
        <span style="color:#909399;">{{ message }}</span>
      </div>
    </el-card>
    <div style="text-align: right">
      <el-button type="primary" round @click="viewThreads()"
        >浏览{{ genderName }}的全部主题 <i class="el-icon-search"></i
      ></el-button>
    </div>
  </el-dialog>
</template>
...
export default {
                    ...
                    this.axios
                      ...
                      .then(response => {
                        ...
                        if (!isNull(vm.mobilePhone))
                          msg += "飞鸽传书：" + vm.mobilePhone + "\r\n";
                        if (!isNull(vm.signature))
                          msg += "人生格言：" + vm.signature;
                        this.message = msg;
                        this.loading = false;
                      });
                  } else {
  ...
  mounted() {
    this.uid = this.root.$store.getters.uid;
    this.getProfile();
  }
};
